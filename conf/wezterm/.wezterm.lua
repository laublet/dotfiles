...
--   Cmd + Shift + C          → fast copy of the latest command output (no picker)
...
--   Cmd + Alt + Space        → copy mode (clears stale search on all tab panes); / ? = fresh EditPattern;
--                              Enter validates pattern + auto-selects match; n / Shift+n cycle matches; y copies;
--                              Esc in search_mode or copy_mode clears pattern when quitting search;
--                              Cmd+F = separate scrollback overlay (see cheatsheet)
--   Cmd + d/D                → split horizontal/vertical
--   Cmd + w                  → close pane
--   Cmd + Z                  → undo (shell: Ctrl+_; agent: ESC[127;5u); Neovim insert: kitty SUPER+z)
--   Cmd + Shift + Z          → zoom pane (agent, shell, Neovim — redo vim dans agent : u / Ctrl+r)
...
-- Solid Dracula background only; macOS desktop wallpaper via `just mac-appearance`.
...
config.colors = {
	tab_bar = {
		background = funk.bg,
		active_tab = {
			bg_color = funk.purple,
			fg_color = funk.bg,
			intensity = "Bold",
		},
		inactive_tab = {
			bg_color = funk.current,
			fg_color = funk.fg,
		},
		inactive_tab_hover = {
			bg_color = funk.current,
...
-- opens the file at the right line/column in Neovim. http(s) links (OAuth, logs,
-- gh/glab --web, …) go through Choosy on macOS (same x-choosy:// scheme as Raycast
-- `ff` / conf/raycast/scripts/open-url-firefox.sh). Keeps the default rules
...
-- macOS only: Choosy rules (Jira/GitLab/Notion → Chrome, …).
local function mac_open_candidates()
	local home = os.getenv("HOME") or ""
	return {
		home .. "/.local/bin/mac-open",
		home .. "/dev/perso/dotfiles/bin/mac-open",
		home .. "/dotfiles/bin/mac-open",
	}
end

local function open_url_via_choosy(uri)
	for _, bin in ipairs(mac_open_candidates()) do
		local f = io.open(bin, "r")
		if f then
			f:close()
			wezterm.background_child_process({ bin, uri })
			return
		end
	end
	wezterm.background_child_process({ "open", "x-choosy://open/" .. uri })
end
...
wezterm.on("open-uri", function(_window, pane, uri)
	...
	if not is_linux and uri:match("^https?://") then
		open_url_via_choosy(uri)
		wezterm.log_info("open-uri: Choosy → " .. uri)
		return false
	end
end)
...
-- Copy-mode search pattern is per-pane; highlights linger on inactive panes
-- after a split unless cleared explicitly (wezterm/wezterm#5952).
local function clear_copy_search_pattern(window, pane)
	pcall(function()
		window:perform_action(act.CopyMode("ClearPattern"), pane)
	end)
end

local function clear_copy_search_all_tab_panes(window, active_pane)
	local tab = active_pane:tab()
	if not tab then
		return
	end
	for _, row in ipairs(tab:panes_with_info()) do
		clear_copy_search_pattern(window, row.pane)
	end
end

local function clear_pattern_then_edit()
	return act.Multiple({
...
local function activate_copy_mode_fresh()
	return wezterm.action_callback(function(window, pane)
		clear_copy_search_all_tab_panes(window, pane)
		...
		action = wezterm.action_callback(function(win, pane)
			if is_nvim(pane) then
				...
				if resize_or_move == "resize" then
					...
					-- No neighbor pane → pass Ctrl+arrow to the shell (zsh partial
					-- autosuggest via forward-word, readline, etc.). Previously we
					-- always called ActivatePaneDirection, which swallowed the key.
					local tab = pane:tab()
					local neighbor = tab and tab:get_pane_direction(dir) or nil
					if neighbor then
						clear_copy_search_pattern(win, pane)
						win:perform_action({ ActivatePaneDirection = dir }, pane)
						hide_cursor_until_move()
					else
						local send_mods = (move_mods == "CTRL|SHIFT") and "CTRL|SHIFT" or "CTRL"
						win:perform_action({
							SendKey = {
								key = key,
								mods = send_mods,
...
wezterm.on("update-status", function(window, _)
	...
	if last_status_cache[key] == left then
		return
	end
...
local copy_last_output_action = wezterm.action_callback(function(window, pane)
	...
	if not ok then
		...
		pcall(function()
			flash_status(window, "Copy failed: " .. tostring(err), 4000)
		end)
	end
end)
...
local pick_output_action = wezterm.action_callback(function(window, pane)
	...
	if not ok then
		...
		pcall(function()
			flash_status(window, "Picker failed: " .. tostring(err), 4000)
		end)
	end
end)
...
config.keys = {
	...
	-- Cmd+Shift+C — fast path: copy the latest output direct to clipboard
	-- (status bar flash, no picker). Deliberate chord — avoids accidental
	...
	-- zsh autosuggest: accept ghost text + run (bindkey '\er' in zshrc).
	-- macOS often does not deliver Ctrl+Enter to apps; explicit sequence is reliable.
	{
		key = "Enter",
		mods = "CTRL",
	...
	-- Terminal-only copy/interrupt (see terminal_copy_bindings above).
...
if not is_linux then
	local AGENT_UNDO = "\x1b[127;5u"
	local SHELL_UNDO = "\x1f" -- Ctrl+_ (SendString avoids DecreaseFontSize SendKey clash)
	local KITTY_SUPER_Z = "\x1b[122;9u"
	...
	table.insert(config.keys, {
		...
		action = wezterm.action_callback(function(win, pane)
			if is_nvim(pane) then
				win:perform_action(act.SendString(KITTY_SUPER_Z), pane)
			elseif pane_has_status_process(pane.foreground_process_name) then
				win:perform_action(act.SendString(AGENT_UNDO), pane)
			else
				win:perform_action(act.SendString(SHELL_UNDO), pane)
			end
		end),
	})
end
...
-- Escape → ClearPattern + Close (drop stale highlights). Wezterm's default Enter is PriorMatch (cycles inside
...
if config.key_tables.copy_mode then
	...
	-- Defensive: ensure `/` and `?` open search-mode from copy-mode (fresh pattern).
	table.insert(config.key_tables.copy_mode, { key = "/", mods = "NONE", action = clear_pattern_then_edit() })
	table.insert(config.key_tables.copy_mode, { key = "?", mods = "NONE", action = clear_pattern_then_edit() })
